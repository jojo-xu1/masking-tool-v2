...
def test_folder_direct_children_ignores_subfolders(tmp_path):
    ...
    assert (tmp_path / "out" / "a.txt").read_text(encoding="utf-8").count(REPL_1) == 1
    assert not (tmp_path / "out" / "nested" / "b.txt").exists()
    assert PHRASE_1 in (folder / "a.txt").read_text(encoding="utf-8")
